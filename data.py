...
def get_examples_from_dir(data_dir, max_length, is_line_as_word=False):
    ...
    for fname in os.listdir(data_dir):
        ...
        if not is_line_as_word:
            ...
            words = [hashlib.md5(line.encode('utf8')).hexdigest() for line in lines]
...
def load_data_and_labels(data_dirs, max_document_length, is_line_as_word):
    x_text = []
    y = []
    labels = np.eye(len(data_dirs), dtype=np.int32).tolist() #矩阵（方针：行=列）-->列表
    #print(labels)
    #print(data_dirs)
...
if __name__ == "__main__":
    positive_data_dir = "E:\\fyx_cnn\\data\\train\\pos"
    negative_data_dir = "E:\\fyx_cnn\\data\train\\neg"
    data_dirs = [negative_data_dir, positive_data_dir]
    load_data_and_labels(data_dirs,1000, 0.1)
